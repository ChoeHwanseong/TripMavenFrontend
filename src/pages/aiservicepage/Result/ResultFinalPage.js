...
const ResultFinalPage = () => {
  ...
  const handleGoToSecondPage = () => {
    console.log('handleGoToSecondPage의 result: ',result);
    // navigate 함수를 이용해 데이터를 state로 전달
  ...
  return (
    ...
    <div className={styles.mainResult}>
      ...
      <div className={styles.resultSection}>
        {/* 영상 분석 결과 */}
        {result?.productEvaluation?.length > 0 && (
          <Box className={styles.resultBox}>
            ...
            <Box className={styles.resultText}>
              {result.productEvaluation[0].commentEye || '눈 분석 결과 없음'}
            </Box>
          </Box>
        )}
      ...
      {result?.productEvaluation?.[0] && (
        <Box className={styles.testBox}>
          ...
          <Box className={styles.resultContent}>
            ...
            <Box className={styles.resultText}>
            {result.productEvaluation[0].tone || ' 목소리 톤 분석 결과 없음'}
            </Box>
          </Box>
      ...
      {result?.productEvaluation?.[1] && (
        <Box className={styles.testBox}>
          ...
          <Box className={styles.resultContent}>
            ...
            <Box className={styles.resultText}>
              {result.productEvaluation[1].commentEye || '눈 분석 결과 없음'}
            </Box>
          </Box>
          <Box className={styles.resultContent}>
            <Typography align="center" className={styles.resultTitle}>
            ...
            <Box className={styles.resultText}>
            {result.productEvaluation[1].tone || ' 목소리 톤 분석 결과 없음'}
            </Box>
          </Box>
